...
=#
include("/home/lkeeble/GRSuite/Testing/Mino_time_coordinate_derivs/MinoTime_dλ.jl");
...
function compute_fit(f::Function, df_dt::Function, d2f_dt::Function, d3f_dt::Function, d4f_dt::Function, d5f_dt::Function, d6f_dt::Function, t_range_factor::Float64,
    ...
    dθ_dt=sol[7,:]; dϕ_dt=sol[8,:]; d2r_dt=sol[9,:]; d2θ_dt=sol[10,:]; d2ϕ_dt=sol[11,:]; dt_dλ=sol[12,:]; psi = sol[13,:]; chi=sol[14,:];
    ...
    test_func_data_1_dλ = zeros(size(λ, 1))
    test_func_data_2_dλ = zeros(size(λ, 1))
    ...
    dx_dλ = [Float64[] for i in 1:size(λ, 1)]
    d2x_dλ = [Float64[] for i in 1:size(λ, 1)]
    ...
    @inbounds for i in eachindex(λ)
        test_func_data_1_dλ[i] = df_dt(dx_dλ[i], x[i]) 
        test_func_data_2_dλ[i] = d2f_dt(d2x_dλ[i], dx_dλ[i], x[i]) 
    ...
    println(test_func_data_1_dλ)
    println(test_func_data_2_dλ)
    ##### perform fit #####
    # println("Length(λ) = $(length(λ))")
    xdata = λ; ydata=test_func_data_0; ydata_1 = test_func_data_1_dλ; ydata_2 = test_func_data_2_dλ;
...
function compute_derivatives(nHarm::Int64, nPoints::Int64, a::Float64, p::Float64, e::Float64, θi::Float64, M::Float64, data_path::String, FFT_flag::Bool)
    ...
    # compute f^{(n)}(λ)
    fλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 0)
    df_dλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 1)
    d2f_dλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 2)
    d3f_dλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 3)
    d4f_dλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 4)
    d5f_dλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 5)
    d6f_dλ = FourierFitGSL_Derivs.curve_fit_functional_derivs(xdata, Ω_fit, fit_params, n_freqs, nPoints, 6)
