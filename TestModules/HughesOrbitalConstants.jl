...
p(ra::Float64, rp::Float64) = 2.0ra * rp / (ra + rp)
e(ra::Float64, rp::Float64) = (ra - rp) / (ra + rp)

# auxiliary terms in Eq. 2.7
aux1(a::Float64, E::Float64, L::Float64, Q::Float64) = Q + L^2 - a^2 * (1.0 - E^2)
aux2(a::Float64, E::Float64, L::Float64) = 4a^2 * L^2 * (1.0 - E^2)

# xI ≡ cos(I) = sgn(Lz)sin(θmin) Eq. 1.3; 
xI(a::Float64, E::Float64, L::Float64, Q::Float64) = Q == 0.0 ? sign(L) : sqrt(2) * L / sqrt(aux1(a, E, L, Q) + sqrt((aux1(a, E, L, Q))^2 + aux2(a, E, L)))   # Eq. 2.7

θmin(xI::Float64, L::Float64) = asin(sign(L) * xI)   # inverting Eq. 1.3

function pe_equatorial(a::Float64, E::Float64, L::Float64, M::Float64)
    E2minus1 = E^2 - 1.0

    A0 = 2.0M * (a * E - L)^2 / E2minus1   # Eq. 3.6
    A1 = (a^2 * E2minus1 - L^2) / E2minus1   # Eq. 3.5
    A2 = 2.0M / E2minus1   # Eq. 3.4
...
function pe_generic(a::Float64, E::Float64, L::Float64, Q::Float64, M::Float64)
    E2minus1 = E^2 - 1.0
    A0 = -a^2 * Q / E2minus1   # Eq. 4.5
    A1 = 2.0M * (Q + (a * E - L)^2) / E2minus1   # Eq. 4.4
    A2 = (a^2 * E2minus1 - L^2 - Q) / E2minus1  # Eq. 4.3
    A3 = 2.0M / E2minus1   # Eq. 4.2
    ...
    ra = -M / (2.0 * E2minus1) + sqrtzrc1 + aux
    rp = -M / (2.0 * E2minus1) + sqrtzrc1 - aux
    r3 = -M / (2.0 * E2minus1) - sqrtzrc1 + aux
    r4 = -M / (2.0 * E2minus1) - sqrtzrc1 - aux

    return ra, rp, r3, r4
end

function peθmin(a::Float64, E::Float64, L::Float64, Q::Float64, M::Float64)
    # equatorial
    if Q==0
        ra, rp, r3 = pe_equatorial(a, E, L, M)
        r4 = 0.0
    else
        ra, rp, r3, r4 = pe_generic(a, E, L, Q, M)
    end

    return ra, rp, r3, r4, p(ra, rp), e(ra, rp), θmin(xI(a, E, L, Q), L)
end
end