...
function NIntegrate_trapezium_rule(y::Vector{ComplexF64}, dx::AbstractVector{Float64}, num_points::Int64)
    sum = 0.0
...
OverLapIntegrand(x::Vector{ComplexF64}, x_conj::Vector{ComplexF64}, h::Vector{ComplexF64}, h_conj::Vector{ComplexF64}, freqs::AbstractVector{Float64}) = @. 2 * (x * h_conj + x_conj * h) / S_h(freqs)
OverLapIntegrandMisMatch(x::Vector{ComplexF64}, x_conj::Vector{ComplexF64}, h::Vector{ComplexF64}, h_conj::Vector{ComplexF64}, freqs::AbstractVector{Float64}, t::Float64) = @. 2 * (x * h_conj * (cos(2π * freqs * t) - im * sin(2π * freqs * t)) + x_conj * h * (cos(2π * freqs * t) + im * sin(2π * freqs * t))) / S_h(freqs)

function compute_overlap(x::Vector{ComplexF64}, h::Vector{ComplexF64}, freqs::AbstractVector{Float64})
    df = diff(freqs)
    num_points = length(x)
...
function compute_overlaps_mismatch(x::Vector{ComplexF64}, h::Vector{ComplexF64}, freqs::AbstractVector{Float64}, t::AbstractVector{Float64})
...
# OverLapIntegrand(x::Vector{ComplexF64}, conj_h::Vector{ComplexF64}, freqs::AbstractVector{Float64}) = @. 4 * (x * conj_h) / S_h(freqs)
# OverLapIntegrandMisMatch(x::Vector{ComplexF64}, conj_h::Vector{ComplexF64}, freqs::AbstractVector{Float64}, t::Float64) = @. 4 * (x * conj_h) * (cos(2π * freqs * t) + im * sin(2π * freqs * t))/ S_h(freqs)
