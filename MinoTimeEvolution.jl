...
function compute_kerr_geodesic(a::Float64, p::Float64, e::Float64, θi::Float64, λmax::Float64=3000.0, Δλi::Float64=1.0, reltol::Float64=1e-12, abstol::Float64=1e-10, saveat::Float64=0.5; data_path::String="Results/")
    # orbital parameters
    M = 1.0;
    ...
    # save trajectory- rows are: λ, λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot, tddot, rddot, θddot, ϕddot, columns are component values at different times
    sol = transpose(stack([λ, t, r, θ, ϕ, dr_dt, dθ_dt, dϕ_dt, d2r_dt2, d2θ_dt2, d2ϕ_dt2, dt_dλ]))
    mkpath(data_path)
    ODE_filename=data_path * "Mino_ODE_sol_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_tstep_$(saveat)_T_$(λmax)_tol_$(reltol).txt"
...
# Δ(r::Float64, a::Float64, M::Float64) = r^2 - 2.0M * r + a^2
# R(r::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = (E * (r^2 + a^2) - a * L)^2 - Δ(r, a, M) * (r^2 + (L - a * E)^2 + Q)    # Eq. 2.1
# Theta(θ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = Q - cot(θ)^2 * L^2 - a^2 * cos(θ)^2 * (1.0 - E^2)    # Eq. 2.2
# Phi(r::Float64, θ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = csc(θ)^2 * L + a * E * ((r^2 + a^2) / Δ(r,a,M) - 1.0) - a^2 * L / Δ(r,a,M)    # Eq. 2.3
# T(r::Float64, θ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = E * ((r^2 + a^2)^2 / Δ(r,a,M) - a^2 * sin(θ)^2) + a * L * (1.0 - (r^2 + a^2) / Δ(r,a,M))    # Eq. 2.4

# tdot(λ::Float64, r::Float64, θ::Float64, ϕ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = T(r, θ, E, L, Q, a, M)
# rdot(λ::Float64, r::Float64, θ::Float64, ϕ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = sqrt(R(r, E, L, Q, a, M))
# θdot(λ::Float64, r::Float64, θ::Float64, ϕ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = sqrt(Theta(θ, E, L, Q, a, M))
# ϕdot(λ::Float64, r::Float64, θ::Float64, ϕ::Float64, E::Float64, L::Float64, Q::Float64, a::Float64, M::Float64) = Phi(r, θ, E, L, Q, a, M)
...
#     tdot = MinoEvolution.tdot.(λ, r, θ, ϕ, E, L, C, a, M)
#     rdot = MinoEvolution.rdot.(λ, r, θ, ϕ, E, L, C, a, M)
#     θdot = MinoEvolution.θdot.(λ, r, θ, ϕ, E, L, C, a, M)
#     ϕdot = MinoEvolution.ϕdot.(λ, r, θ, ϕ, E, L, C, a, M)

    
#     # # substitute solution back into geodesic equation to find second derivatives of BL coordinates (wrt λ)
#     # d2t_dτ = KerrGeodesics.tddot.(λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot, params...);
#     # d2r_dτ = KerrGeodesics.rddot.(λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot, params...);
#     # d2θ_dτ = KerrGeodesics.θddot.(λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot, params...);
#     # d2ϕ_dτ = KerrGeodesics.ϕddot.(λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot, params...);

#     # save trajectory- rows are: λ, λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot, tddot, rddot, θddot, ϕddot, columns are component values at different times
#     # sol = transpose(stack([λ, r, θ, ϕ, dt_dλ, dr_dλ, dθ_dλ, ϕdot, d2t_dτ, d2r_dτ, d2θ_dτ, d2ϕ_dτ, r, θ, rdot, θdot]))
#     sol = transpose(stack([λ, λ, r, θ, ϕ, tdot, rdot, θdot, ϕdot]))
#     mkpath(data_path)
#     ODE_filename=data_path * "Mino_ODE_sol_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_tstep_$(saveat)_T_$(λmax)_tol_$(reltol).txt"
#     open(ODE_filename, "w") do io
#         writedlm(io, sol)
