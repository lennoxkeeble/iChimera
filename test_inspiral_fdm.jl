...
data_path="Test_results_fdm/";
plot_path="Test_plots_fdm/";
mkpath(data_path)
mkpath(plot_path)
...
h=0.2;
# compute inspiral
@time SelfForce_numerical.compute_inspiral_HJE!(τMax_geometric, nPoints, M, m, a, p, e, θi,  Γαμν, g_μν, g_tt, g_tϕ, g_rr, g_θθ, g_ϕϕ, gTT, gTΦ, gRR, gThTh, gΦΦ, h, saveat_geometric, Δti, kerrReltol, kerrAbstol, data_path=data_path)

# # plot orbit solution
# EMRI_ode_sol_fname = data_path * "EMRI_ODE_sol_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_q_$(m/M)_tstep_$(saveat_geometric)_tol_$(kerrReltol)_fdm.txt"
# EMRI_orbit_fname = "EMRI_3d_orbit_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_q_$(m/M)_tstep_$(saveat_geometric)_tol_$(kerrReltol)_fdm.txt"
# zlims = (-30, 30);
...
# load ODE solution
ODE_filename=data_path * "EMRI_ODE_sol_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_q_$(m/M)_tstep_$(saveat_geometric)_tol_$(kerrReltol)_fdm.txt"
...
constants_filename=data_path * "constants_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_q_$(m/M)_tstep_$(saveat_geometric)_tol_$(kerrReltol)_fdm.txt"
constants=readdlm(constants_filename)
constants_derivs_filename=data_path * "constants_derivs_a_$(a)_p_$(p)_e_$(e)_θi_$(round(θi; digits=3))_q_$(m/M)_tstep_$(saveat_geometric)_tol_$(kerrReltol)_fdm.txt"
